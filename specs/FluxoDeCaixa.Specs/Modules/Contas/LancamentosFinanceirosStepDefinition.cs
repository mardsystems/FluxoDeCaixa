...
namespace FluxoDeCaixa.Modules.Contas
{
    ...
    public sealed class LancamentosFinanceirosStepDefinition
    {
        ...
        [Given(@"que eu tenho uma conta '(.*)' \(banco: '(.*)', conta Corrente: '(.*)', cpf: '(.*)'\)")]
        public void DadoQueEuTenhoUmaContaBancoContaCorrenteCpf(string p0, int p1, int p2, int p3)
        {
            ScenarioContext.Current.Pending();
        }

        [Given(@"que a minha conta hoje tem saldo de R\$ (.*)")]
        public void DadoQueAMinhaContaHojeTemSaldoDeR(Decimal p0)
        {
            ScenarioContext.Current.Pending();
        }

        [When(@"lanço um pagamento de '(.*)' de R\$ (.*) hoje na minha conta sob o protocolo '(.*)'")]
        public void QuandoLancoUmPagamentoDeDeRHojeNaMinhaConta(string p0, Decimal p1, string p2)
        {
            ScenarioContext.Current.Pending();
        }

        [Then(@"o saldo da minha conta deve ser de R\$ (.*)")]
        public void EntaoOSaldoDaContaDeveSerDeR(Decimal p0)
        {
            ScenarioContext.Current.Pending();
        }

        [Then(@"um lançamento '(.*)' no valor de R\$ (.*) deve ser adicionado na minha conta com o protocolo '(.*)'")]
        public void EntaoUmLancamentoNoValorDeRDeveSerAdicionadoNaConta(string p0, Decimal p1, string p2)
        {
            ScenarioContext.Current.Pending();
        }
